...
public class TFIDFCalculator
{
    ...
    private static String IDF = "idf.txt";
    private static String SEPERATEOR_REGEX = "\\t";
    ...
    private MeanWordFilter filter = new MeanWordFilter();
    private Weight.TitleWeight titleWeight = new Weight.TitleWeight(5, filter);
    private Weight.ContentWeight contentWeight = new Weight.ContentWeight(3, filter);
...
//        System.out.println(System.getProperty("java.class.path"));
    ...
    public static class Fragment
    {
        ...
        public Fragment(String data, Weight weighter)
        {
            this.data = data;
    ...
    public List<KeyWord> computeTfidf(String title, String content, int kwCount)
    {

        Map<String, KeyWord> kws = Maps.newHashMap();

        computeTfidfInternal(title, kws, titleWeight);
        computeTfidfInternal(content, kws, contentWeight);
    ...
    private void computeTfidfInternal(String title, Map<String, KeyWord> kws, Weight weighter)
    {
        List<Term> words = ToAnalysis.parse(title);

        for (Term word : words)
        {
            int weight = (int) weighter.computeWeight(word, title.length());
